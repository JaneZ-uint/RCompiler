...
namespace JaneZ {
class ConstCalculator : public ASTVisitor {
public:
    void visit(ASTNode &node) override{
        if(auto *p = dynamic_cast<ASTRootNode *>(& node)){
            visit(*p);
        }else{
            throw std::runtime_error("ConstCalculator can only visit ASTRootNode");
        }
    }
    ...
    void visit(Expression &node) override{
        if(auto *p = dynamic_cast<ExprArray *>(& node)){
            ...
            throw std::runtime_error("ConstCalculator::visit(Expression&): unknown derived class of Expression");
        }
    }
    ...
    void visit(Item &node) override{
        if(auto *p = dynamic_cast<ItemConstDecl *>(& node)){
            ...
            throw std::runtime_error("ConstCalculator::visit(Item&): unknown derived class of Item");
        }
    }
