...
public:
    ...
    bool isReferencePattern(Pattern *pattern){
    ...
    bool isReferencePatternFunctionParam(ItemFnDecl &node, int irParamIndex){
        ...
        return isReferencePattern(node.fnParameters.FunctionParam[userParamIndex].pattern.get());
    }
    ...
    std::shared_ptr<IRVar> bindReferencePatternParam(const std::shared_ptr<IRVar> &param,
        ...
        auto ptrValue = std::make_shared<IRVar>();
        ptrValue->type = param->type;
        auto loadPtr = std::make_shared<IRLoad>();
        loadPtr->tmp = ptrValue;
        loadPtr->addressVar = slot;
        loadPtr->type = param->type;
        loadPtr->w64tag = true;
        ...
        if (node.ret != nullptr && node.is_lvalue == false) {
            if(auto var = std::dynamic_pointer_cast<IRVar>(node.ret)){
                if(auto *inttype = dynamic_cast<IRIntType *>(& *var->type)){
                    ...
                    auto appendInstr = [&](std::shared_ptr<IRNode> instr){
                        if(ret->blockList.empty()){
                            ret->instrList.push_back(instr);
                        }else{
                            auto lastBlock = ret->blockList[ret->blockList.size() - 1];
                    ...
                    if(var->isPtrStorage){
                        ...
                        loadInstr->tmp = tmpVar;
                        loadInstr->addressVar = var;
                        loadInstr->type = ptrtype->baseType;
                        if(inttype->bitWidth == 64){
                            loadInstr->w64tag = true;
                        }
                        if(inttype->bitWidth == 32 && inttype->isUnsigned){
                        ...
                        appendInstr(std::make_shared<IRAlloca>(tmpVar->type,tmpVar));
                        appendInstr(memcpyInstr);
                        node.ret = tmpVar;
                    }else{
                        auto loadInstr = std::make_shared<IRLoad>();
                        auto tmpVar = std::make_shared<IRVar>();
    ...
    std::shared_ptr<IRBlock> visit(ExprArray &node){
        ...
        if(node.arrayExpr.size() == 0){
            ...
            if(node.type->ret && dynamic_cast<IRLiteral *>(& *node.type->ret)){
                ...
                assignBlock->instrList.push_back(getptrInstr);
                block->instrList.push_back(std::make_shared<IRAlloca>(arrayVar->type,arrayVar));
                if(std::dynamic_pointer_cast<IRPtrType>(var->type)){
                    auto storeInstr = std::make_shared<IRStore>();
                    storeInstr->valueType = resVar->type;
                    storeInstr->address = getptrInstr->res;
    ...
    std::shared_ptr<IRBlock> visit(ExprField &node){
        auto block = std::make_shared<IRBlock>();
        std::shared_ptr<IRVar> var = nullptr;
        if(auto *path = dynamic_cast<ExprPath *>(& *node.expr)){
            var = currentScope->lookupValueSymbol(path->pathFirst->pathSegments.identifier);
        }
        if(var){
            if(auto *ptrtype = dynamic_cast<IRPtrType *>(& *var->type)){
                node.expr->is_lvalue = false;
            }
        }
        ...
        if(auto var = std::dynamic_pointer_cast<IRVar>(obj)){
            ...
            if(auto structType = std::dynamic_pointer_cast<IRStructType>(var->type)){
                ...
                if(auto structType = std::dynamic_pointer_cast<IRStructType>(ptrType->baseType)){
                    for(int i = 0;i < structType->memberTypes.size();i ++){
                        auto & member = structType->memberTypes[i];
                        if(member.first == node.identifier){
                            auto getptrInstr = std::make_shared<IRGetptr>();
    ...
    std::shared_ptr<IRBlock> visit(ExprIf &node, std::shared_ptr<IRVar> structRetVar = nullptr){
        ...
        if(node.condition){
            ...
            if(auto var = std::dynamic_pointer_cast<IRVar>(node.condition->ret)){
                ...
                if(literal->literalType == BOOL_LITERAL){
                    auto allocVar = std::make_shared<IRVar>();
                    allocVar->type = currentScope->lookupTypeSymbol("BOOL");
                    auto allocInstr = std::make_shared<IRAlloca>(allocVar->type,allocVar);
                    auto storeInstr = std::make_shared<IRStore>();
                    storeInstr->valueType = allocVar->type;
                    storeInstr->address = allocVar;
                    storeInstr->storeLiteral = std::make_shared<IRLiteral>(BOOL_LITERAL,literal->intValue);
                    auto loadInstr = std::make_shared<IRLoad>();
                    loadInstr->tmp = condVar;
                    loadInstr->addressVar = allocVar;
                    loadInstr->type = allocVar->type;
                    if(condBlock->blockList.empty()){
    ...
    std::shared_ptr<IRBlock> visit(ExprLoop &node){
        ...
        if(node.condition){
            ...
            if(auto var = std::dynamic_pointer_cast<IRVar>(node.condition->ret)){
                ...
                if(literal->literalType == BOOL_LITERAL){
                    auto var = std::make_shared<IRVar>();
                    var->type = currentScope->lookupTypeSymbol("BOOL");
                    auto allocaInstr = std::make_shared<IRAlloca>();
                    allocaInstr->var = var;
                    allocaInstr->allocatedType = currentScope->lookupTypeSymbol("BOOL");
                    auto storeInstr = std::make_shared<IRStore>();
                    storeInstr->address = var;
                    ...
                    loadInstr->addressVar = var;
                    loadInstr->tmp = condVar;
                    loadInstr->type = currentScope->lookupTypeSymbol("BOOL");
                    if(condBlock->blockList.empty()){
                        condBlock->instrList.push_back(allocaInstr);
                        condBlock->instrList.push_back(storeInstr);
                        condBlock->instrList.push_back(loadInstr);
                    }else{
                        auto lastBlock = condBlock->blockList[condBlock->blockList.size() - 1];
                        lastBlock->instrList.push_back(allocaInstr);
                        lastBlock->instrList.push_back(storeInstr);
                        lastBlock->instrList.push_back(loadInstr);
                    }
                }
    ...
    std::shared_ptr<IRBlock> visit(ExprOpbinary &node){
        ...
        if(auto *right = dynamic_cast<ExprPath *>(& *node.right)){
            ...
            if(res.type != "null"){
                ...
                }else if(auto *left = dynamic_cast<ExprOpunary *>(& *node.left)){
                    visit(*left);
                    if(auto lit = std::dynamic_pointer_cast<IRLiteral>(left->ret)){
                        leftnum = lit->intValue;
                        if(node.op == PLUS){
                            node.ret = std::make_shared<IRLiteral>(INT_LITERAL, leftnum + rightnum);
                            node.is_lvalue = true;
                            return nullptr;
        ...
        if(op == ADD_EQ || op == SUB_EQ || op == MUL_EQ || op == DIV_EQ || op == MOD_EQ || op == XOROPEQ || op == LEFTSHIFTOPEQ || op == RIGHTSHIFTOPEQ){
            ...
            if(auto leftvar = std::dynamic_pointer_cast<IRVar>(leftExpr)){
                ...
                }else if(auto ptrType = std::dynamic_pointer_cast<IRPtrType>(leftvar->type)){
                    if(auto pointeeInt = std::dynamic_pointer_cast<IRIntType>(ptrType->baseType)){
                        auto storeInstr = std::make_shared<IRStore>();
                        storeInstr->valueType = ptrType->baseType;
                        if(auto rightLiteral = std::dynamic_pointer_cast<IRLiteral>(rightExpr)){
                            storeInstr->storeLiteral = rightLiteral;
                            assignResult = rightLiteral;
                        }else if(auto rightVar = std::dynamic_pointer_cast<IRVar>(rightExpr)){
                            storeInstr->storeValue = rightVar;
                            assignResult = rightVar;
                        }
                        storeInstr->address = leftvar;
                        if(pointeeInt->bitWidth == 64){
                            storeInstr->w64tag = true;
                        }
                        if(block->blockList.size() == 0){
                            block->instrList.push_back(storeInstr);
            ...
            if(auto leftvar = std::dynamic_pointer_cast<IRVar>(leftExpr)){
                ...
                }else if(auto rightLiteral = std::dynamic_pointer_cast<IRLiteral>(rightExpr)){
                    if(leftvar->type == currentScope->lookupTypeSymbol("bool")){
                        binaryInstr->i8tag = true;
                    }
                }
    ...
    std::shared_ptr<IRBlock> visit(ExprOpunary &node){
        ...
        if(node.op == DEREFERENCE){
            ...
            if(auto rightVar = std::dynamic_pointer_cast<IRVar>(node.right->ret)){
                bool borrowDeref = false;
                if(auto *rightDeref = dynamic_cast<ExprOpunary *>(& *node.right)){
                    borrowDeref = rightDeref->op == DEREFERENCE;
                }
                if(borrowDeref && std::dynamic_pointer_cast<IRPtrType>(rightVar->type)){
                    node.ret = rightVar;
                }else{
                    auto ptrValue = std::make_shared<IRVar>();
                    ptrValue->type = std::make_shared<IRPtrType>(rightVar->type);
                    auto moveInstr = std::make_shared<IRStore>();
                    moveInstr->valueType = ptrValue->type;
                    moveInstr->storeValue = rightVar;
                    moveInstr->address = ptrValue;
                    moveInstr->w64tag = true;
                    if(block->blockList.empty()){
                        block->instrList.push_back(moveInstr);
                    }else{
                        block->blockList[block->blockList.size() - 1]->instrList.push_back(moveInstr);
                    }
                    node.ret = ptrValue;
                }
            }else{
        ...
        for(int i = 0;i < currentIRFunc->paramList->paramList.size();i++){
            ...
            if(auto p = std::dynamic_pointer_cast<IRVar>(param)){
                ...
                if(auto *structType = dynamic_cast<IRStructType *>(& *p->type)){
                    ...
                    bool destructureReferencePattern = isReferencePatternFunctionParam(node, i);
                    bool bindReferenceSlot = !destructureReferencePattern &&
                                             isMutableReferenceFunctionParam(node, i);
                    ...
                    if(destructureReferencePattern){
                        auto local = bindReferencePatternParam(
                            p,
            ...
            for(int i = 0;i < node.fnBody->statements.size();i++){
                ...
                if(auto *p = dynamic_cast<StmtExpr *>(& *stmt)){
                    ...
                    if(auto *q = dynamic_cast<ItemFnDecl *>(& *p->stmt_item)){
                        auto childFunc = visit(*q);
                        currentIRFunc->funcList.push_back(std::dynamic_pointer_cast<IRFunction>(childFunc));
                    }else if(auto *q = dynamic_cast<ItemStructDecl *>(& *p->stmt_item)){
                        auto structDecl = visit(*q);
                        currentIRFunc->structTypeList.push_back(std::dynamic_pointer_cast<IRStructType>(structDecl));
                    }
                }
            }
    ...
    std::shared_ptr<IRBlock> visit(StmtLet &node){
        ...
        if(node.expression){
            if(auto *ifExpr = dynamic_cast<ExprIf *>(& *node.expression)){
                ...
                }else if(auto *tp = dynamic_cast<IRArrayType *>(& *varType)){
                    node.expression->is_lvalue = true;
                }else if(auto *tp = dynamic_cast<IRPtrType *>(& *varType)){
                    node.expression->is_lvalue = true;
                }
