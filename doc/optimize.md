...
CR 时后端建议只讲下面几项。`InstrSelect`、ASM DCE、普通 immediate 选择等仍在代码里，但收益更像基础清理，不再单独作为重点优化展开。

### 4.1 Modular Helper Inline 和关键 Peephole
...
- 将一次性地址临时折入 load/store，例如 `addi addr, base, off; ld x, 0(addr)` 改成 `ld x, off(base)`。
- 删除 self move 和 `addi x, x, 0`。

适用场景：

- NTT 里的模加/模减。
- graph 里的 `norm` helper。
- 状态机/解释器中 bool 比较后马上分支。
- 数组访问中临时地址只用一次。

核心收益：

- 减少热点 helper call。
- 减少 bool 临时寄存器和短命地址寄存器。
- 降低后续寄存器分配压力。

### 4.2 Linear Scan Register Allocation 改进
...
- 按函数构建 CFG，计算 liveIn/liveOut，而不是只看线性文本顺序。
- 使用 live ranges 判断冲突，允许不重叠 range 共用物理寄存器。
- 对循环块加权，让热点循环中的 use 有更高 spill cost。
- 对跨 call 的 interval 优先尝试 callee-saved 寄存器。
- call 只把实际使用的 `a0-a7` 参数寄存器计为 use。
- 收集 `mv v1, v2` copy hints，尽量把相关虚拟寄存器分到同一个物理寄存器。
- 对单定义 `li` 形式的常量做 rematerialization，避免真实 spill slot。
...
- 比朴素 linear scan 更少 spill/reload。
...
- next-use spill 选择和 spill slot 复用都尝试过但回退，当前文档不作为主线介绍。
...
- 小函数调用密集样例中减少 prologue/epilogue。
- leaf helper 不再为 `ra/s0` 做无意义保存恢复。

### 4.4 Post-regalloc Spill/Stack Cleanup
...
- 清理 linear scan 之后残留的冗余 spill/reload。
- 减少大 frame 下重复 materialize 栈地址的开销。
- 对图、解释器、索引类大函数比较直接。
